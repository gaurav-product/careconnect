...
**Status: 60 unit and integration tests + 24 end-to-end journeys (12 scenarios × mobile and
desktop). All passing. No console or page errors on any screen.**

```
npm test          # 60 tests, ~4s
npm run test:e2e  # 24 journeys across Pixel-7 and desktop viewports
```
...
| Verified care day | Complete day passes; missed critical fails; unrecorded critical fails; open urgent alert fails; resolved urgent alert passes; below the 80% floor fails; an empty day never counts |
...
| Metrics | Computed within bounds; future and malformed dates rejected |
...
9–12. Failure states: wrong password, unknown invite code, unknown URL, deep link while
signed out.
...
| Is core value delivered in minutes? | Setup is ~5 minutes; the first verified day arrives after one shift. Seeded data shows the end state immediately |
